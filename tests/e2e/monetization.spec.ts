...
test.describe('Monetization - Stripe Checkout', () => {
  test('Hits Humanizer limit, upgrades to Pro via Stripe, and cancels', async ({ page }, testInfo) => {
    ...
    const writeModeBtn = t(dict, 'Dashboard.Sidebar.write');
    await page.getByText(writeModeBtn).click();
    ...
    const humanizeBtn = t(dict, 'Dashboard.Components.humanize');
    await page.getByRole('button', { name: humanizeBtn }).click();

    // 4. Paywall should appear
    const paywallTitle = t(dict, 'Dashboard.Components.upgradeToPro', 'Upgrade to Pro');
    await expect(page.getByText(paywallTitle)).toBeVisible();
    ...
    const settingsBtn = t(dict, 'Dashboard.Sidebar.settings');
    await page.getByText(settingsBtn).click();
    
    // Wait for Settings page
    await page.waitForURL(`**/${locale}/settings`);
    
    const manageBillingBtn = t(dict, 'Dashboard.Settings.manageBilling');
    ...
    await page.getByRole('button', { name: manageBillingBtn }).click();
