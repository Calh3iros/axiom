    ...
    <div className="flex h-full gap-6 relative">
      ...
      >
        <div className="p-4 border-b border-[var(--color-border)] flex items-center justify-between">
          <h2 className="font-bold text-[var(--color-text-primary)]">Chat History</h2>
        </div>
        ...
        <div className="p-4 pb-2">
          ...
          >
            <Plus className="w-4 h-4" /> New Solve
          </button>
        </div>

        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {chats.length === 0 ? (
            <p className="text-sm text-[var(--color-dim)] text-center mt-4">No recent chats</p>
          ) : (
            chats.map(chat => (
              <button
                key={chat.id}
